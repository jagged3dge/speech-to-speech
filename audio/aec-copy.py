...
class AcousticEchoCanceller:
    ...
    def _initialize_speex(self):
        ...
        try:
            ...
            result = self.speex.speex_echo_ctl(
                self.state, 
                0,  # SPEEX_ECHO_SET_SAMPLING_RATE
                byref(c_int(self.sample_rate))
            )
            if result != 0:
                raise RuntimeError(f"Failed to set sample rate: error {result}")
            
            # Set filter adaptation rate
            filter_adapt = c_float(0.1)
            result = self.speex.speex_echo_ctl(
                self.state, 
                11,  # SPEEX_ECHO_SET_FILTER_ADAPTATION
                byref(filter_adapt)
            )
            if result != 0:
                raise RuntimeError(f"Failed to set adaptation rate: error {result}")
    ...
    def process(self, mic_signal, speaker_signal):
        """Process audio chunks with echo cancellation."""
        if not self.enabled:
            return mic_signal
