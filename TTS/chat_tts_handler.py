...
class ChatTTSHandler(BaseHandler):
    def setup(
        self,
        should_listen,
        interrupt_event=None,  # Add interrupt event parameter
        device="cuda",
        gen_kwargs={},  # Unused
        stream=True,
        chunk_size=512,
    ...
    def process(self, llm_sentence):
        ...
        try:
            ...
            if self.stream:
                ...
                for gen in wavs_gen:
                    ...
                    if self.interrupt_event and self.interrupt_event.is_set():
                        logger.info("ChatTTS interrupted by user speech")
                    ...
                    while len(audio_chunk) > self.chunk_size:
                        ...
                        if self.interrupt_event and self.interrupt_event.is_set():
                            logger.info("ChatTTS interrupted during playback")
                            self.interrupt_event.clear()
                            self.should_listen.set()
                            return
                ...
                # Non-streaming mode with interruption support
                wavs = wavs_gen
                if len(wavs[0]) == 0:
                    self.should_listen.set()
                    return
            ...
            logger.error(f"Error in ChatTTSHandler: {e}")
        finally:
            self.should_listen.set()
